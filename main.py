...
# Creer une route stndard avec la version pour toutes les methodes
...
@router_v2.get("/etudiants/", response_model=list[schemas.Etudiants])
...
@router_v2.get("/etudiants/{id_recherche}",response_model=schemas.Etudiants)
...
@router_v2.post("/etudiants/", response_model=schemas.Etudiants, status_code=status.HTTP_201_CREATED)
def creer_etudiant(etudiant: schemas.EtudiantsCreate, db: Session = Depends(get_db)):
